...
(async ()=> {
  ...
  var devnetPeers, mainnetPeers;
  try {
    const jsonDevnetPeers = await requestPromise('https://raw.githubusercontent.com/ArkEcosystem/ARK-Peers/master/devnet.json');
    devnetPeers = JSON.parse(jsonDevnetPeers).map(objIpPort => objIpPort.ip + ':' + objIpPort.port);

    const jsonMainnetPeers = await requestPromise('https://raw.githubusercontent.com/ArkEcosystem/ARK-Peers/master/mainnet.json');
    mainnetPeers = JSON.parse(jsonMainnetPeers).map(objIpPort => objIpPort.ip + ':' + objIpPort.port);
  }
  catch (e) {
    console.error(e);
    throw 'Could not fetch peers from Github.';
  }
  const networks = {
    devnet: {
