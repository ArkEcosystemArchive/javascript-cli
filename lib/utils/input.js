...
prompt.message = chalk.blue.bold('ARK-Client')
...
class Input {
  ...
  async promptConfirmTransaction (transaction) {
    const value = parseFloat(transaction.amount) / ARKUnits
    const recepient = transaction.recipientId
    const promptSchema = {
      properties: {
        confirm: {
          description: 'Confirm your transaction',
          message: `Sending ${this.symbol} ${value} to ${recepient} now. Are you sure? Y(es)/N(o)`,
          required: true,
          conform: function (value) {
