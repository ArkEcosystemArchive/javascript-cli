...
module.exports = async (signature, cmd) => {
  ...
  try {
    ...
    const promptSignature = typeof (signature === 'undefined')
    const promptPassphrase = !cmd.passphrase || cmd.passphrase === true
    const inputResponse = await input.getPrompt(promptPassphrase, false, promptSignature)
    ...
    if (inputResponse.hasOwnProperty('signature')) {
      signature = inputResponse.signature.toString()
    }
    ...
    const transaction = arkjs.signature.createSignature(passphrase, signature);

    // Execute the transaction
    if (interactive) {
      // Promt to confirm transaction
      const message = `Creating second signature: ${signature} now. Are you sure? Y(es)/N(o)`
