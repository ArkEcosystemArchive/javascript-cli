...
module.exports = async (username, cmd) => {
  ...
  try {
    ...
    // Test if this delegate doesn't already exist
    let delegateExists = false
    try {
      await accountUtils.isValidDelegate(network, username)
      delegateExists = true
    } catch (err) {
      // this is good, we didn't find a delegate <username>
    }

    if (delegateExists) {
      throw new Error(`Unable to register: a delegate with username: ${username} already exists on ${net}.`)
    }
