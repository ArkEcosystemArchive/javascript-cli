...
class Network {
  ...
  async connect (network) {
    ...
    try {
      const response = await this.getFromNode('/api/node/configuration')
  ...
  async findAvailablePeers () {
    try {
      const response = await this.getFromNode('/api/peers')
      ...
      for (let i = 0; i < peers.length; i++) {
        ...
        if (reachable) {
          ...
          commands[queue].push(new Promise((resolve, reject) => {
            this.getFromNode('/api/node/syncing', {}, peers[i])
            .then((response) => {
              if (typeof (response) !== 'undefined' && response.data.hasOwnProperty('data')) {
